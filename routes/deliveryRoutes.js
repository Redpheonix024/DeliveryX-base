...
router.post('/submitDeliveryDetails', async (req, res) => {
  try {
    const { currentLocation, travelingLocation, meansOfTravel, startDate, startTime, arrivalDate, arrivalTime } = req.body;
    if (currentLocation === travelingLocation) {
      console.error('Current location and traveling location cannot be the same.');
      return;
    }
    const username = req.session.username;
    const newDelivery = new Delivery({
      username, currentLocation, travelingLocation, meansOfTravel,
      startDate, startTime, arrivalDate, arrivalTime
    });
    await newDelivery.save();
    res.status(200).json({ message: 'Delivery details successfully submitted.' });
  } catch (error) {
...
router.get('/get-delentries', async (req, res) => {
    try {
      ...
      const entries = await Delivery.find({ username }).sort({ submitTime: -1 });
  ...
  router.post('/update-delentry/:submissionDateTime',  async (req, res) => {
    ...
    try {
      ...
      const entryToUpdate = await Delivery.findOne({ submissionDateTime: entrysubmittime });
...
router.get('/get-delev-count', isAuthenticated, async (req, res) => {
    try {
      const username = req.session.username;
      const entryCount = await Delivery.countDocuments({ username });
  ...
  router.get('/get-delentry/:entrysubmittime', async (req, res) => {
    ...
    try {
      const entry = await Delivery.findOne({ submissionDateTime: new Date(decodedSubmitTime) });
  
      if (!entry) {
        console.log('Entry not found for query:', { submissionDateTime: new Date(decodedSubmitTime) });
  ...
  router.delete('/delete-delentry/:submissionDateTime', async (req, res) => {
    ...
    try {
      ...
      const deletedEntry = await Delivery.findOneAndDelete({ submissionDateTime });
